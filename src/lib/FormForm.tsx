...
export type FormBaseProps = Omit<
  AntdFormProps,
  | 'initialValues'
  | 'form'
  | 'children'
...
function FormWithFormik<TValue extends FormikValues = any>(props: FormWithFormikProps<TValue>) {
  ...
  return (
    <FormFormik<TValue> {...formikProps} onSubmit={handleSubmit}>
      <AntdForm {...formProps}>{children as ReactElement}</AntdForm>
      {onInvalidSubmit && <FormInvalidSubmit onInvalidSubmit={onInvalidSubmit} />}
    </FormFormik>
  );
}

const FormFormComponent = memo(<TValue extends FormikValues = any>(props: FormProps<TValue>) => {
  props = Object.assign({}, FormForm.defaults, props);
...
}) as (<TValue extends FormikValues = any>(props: FormProps<TValue>) => ReactElement | null) & {
  displayName?: string;
};

FormFormComponent.displayName = 'Form';

export const FormForm = /* @__PURE__ */ Object.assign(fc(FormFormComponent), {
  defaults: {} as Partial<FormProps<any>>,
});
