...
const FormInputComponent = memo((props: FormInputProps) => {
  props = Object.assign({}, FormInput.defaults, props);

  const { name: nameProp, mapGet, mapSet, ...inputProps } = props;
...
const DEFAULTS: Pick<FormInputProps, 'mapSet' | 'mapGet'> = {
  mapGet: ([{ value = '' }]) => value,
  mapSet: (e) => (e.target.value?.length === 0 ? null : e.target.value ?? null),
};

FormInputComponent.displayName = 'Form.Input';

export const FormInput = /* @__PURE__ */ Object.assign(FormInputComponent, {
  defaults: DEFAULTS,
});
